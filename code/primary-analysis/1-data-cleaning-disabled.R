...
person <- cps.p%>%
  filter(PRDISFLG == 1)%>% #15,910 individuals with disabilities
  filter(AGE1 >= 3)%>% #Remove all people below age 18
  select(PH_SEQ, A_HGA, AGE1, A_SEX, PRCITSHP, A_MARITL, PEDISDRS, 
         PEDISEAR, PEDISEYE, PEDISOUT, 
         PEDISPHY, PEDISREM, PEHSPNON, PRDTRACE, PEAFEVER,
...
#PEDISDRS Dummy
final$Dis_Dress_yes <- ifelse(final$PEDISDRS == '1', 1, 0)

#PEDISEAR Dummy
final$Dis_Hearing_yes <- ifelse(final$PEDISEAR == '1', 1, 0)

#PEDISEYE Dummy
final$Dis_Visual_yes <- ifelse(final$PEDISEYE == '1', 1, 0)

#PEDISOUT Dummy
final$Dis_Other_yes <- ifelse(final$PEDISOUT == '1', 1, 0)

#PEDISPHY Dummy
final$Dis_Physical_yes <- ifelse(final$PEDISPHY == '1', 1, 0)
...
final1<- final%>%
  select(-c(A_HGA, AGE1, A_SEX, PRCITSHP, A_MARITL, 
            ...
            PEDISREM, PEHSPNON, PRDTRACE, PEAFEVER, 
